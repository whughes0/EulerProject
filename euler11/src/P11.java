    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i <= 19; i++) {
            for (int j = 0; j <= 19; j++) {
                //multiply right
                if (j <= 16) {
                    number = array[i][j] * array[i][j + 1] * array[i][j + 2]
                            * array[i][j + 3];
                    if (max < number) {
                        max = number;
                        for (int k = 0; k < 4; k++) {
                            maxarray[k] = array[i][j + k];
                        }
                    }
                }
                ...
                //multiply down
                if (i <= 16) {
                    number = array[i][j] * array[i + 1][j] * array[i + 2][j]
                            * array[i + 3][j];
                    if (max < number) {
                        max = number;
                        for (int k = 0; k < 4; k++) {
                            maxarray[k] = array[i + k][j];
                        }
                    }
                }

                //multiply diagonal down right
                if (i <= 16 && j <= 16) {
                    number = array[i][j] * array[i + 1][j + 1]
                            * array[i + 2][j + 2] * array[i + 3][j + 3];
                    if (max < number) {
                        max = number;
                        for (int k = 0; k < 4; k++) {
                            maxarray[k] = array[i + k][j + k];
                        }
                    }
                }

                //multiply diagonal down left
                if (i <= 16 && j >= 3) {
                    number = array[i][j] * array[i + 1][j - 1]
                            * array[i + 2][j - 2] * array[i + 3][j - 3];
//                    System.out.println( array[i][j]  array[i + 1][j - 1]
//                            * array[i + 2][j - 2] * array[i + 3][j - 3];);
